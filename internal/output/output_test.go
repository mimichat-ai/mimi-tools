...
func TestTruncateExecOutput_ShortOutput(t *testing.T) {
	// Output under the limit should be returned unchanged
	s := strings.Repeat("line\n", 50) // 50 lines
	got := TruncateExecOutput(s)
	if got != s {
		t.Error("expected unchanged output for 50 lines")
	}
}

func TestTruncateExecOutput_ExactLimit(t *testing.T) {
	// Exactly execTruncateLines (210) lines should be unchanged.
	// Use n-1 lines with trailing newline + 1 without, so SplitLines yields exactly 210.
	s := strings.Repeat("line\n", execTruncateLines-1) + "line"
	got := TruncateExecOutput(s)
	if got != s {
		t.Errorf("expected unchanged output for exactly %d lines", execTruncateLines)
	}
}

func TestTruncateExecOutput_OneOverLimit(t *testing.T) {
	// 211 lines: should truncate 1 line
	total := execTruncateLines + 1
	...
	wantMsg := "... (truncated 1 lines) ..."
	if !strings.Contains(got, wantMsg) {
		t.Errorf("expected truncation message %q, got:\n%s", wantMsg, got)
	}

	// Should contain the first headLines lines
	if !strings.Contains(got, lines[0]) {
		t.Error("expected first line to be present in truncated output")
	}
	if !strings.Contains(got, lines[headLines-1]) {
		t.Error("expected last head line to be present in truncated output")
	}

	// Should contain the last tailLines lines
	if !strings.Contains(got, lines[total-1]) {
		t.Error("expected last line to be present in truncated output")
	}
	if !strings.Contains(got, lines[total-tailLines]) {
		t.Error("expected first tail line to be present in truncated output")
	}

	// Should NOT contain the middle line that was truncated
	middle := lines[headLines]
	if strings.Contains(got, middle) {
		t.Error("expected middle line to be absent from truncated output")
	}
}

func TestTruncateExecOutput_LargeOutput(t *testing.T) {
	// 1000 lines: should truncate 790 lines
	total := 1000
	lines := make([]string, total)
	for i := range lines {
		lines[i] = "line" + itoa(i)
	...
	for i := 0; i < headLines; i++ {
		if !strings.Contains(got, lines[i]) {
			t.Errorf("expected head line %d (%q) to be present", i, lines[i])
		}
	}
	...
	for i := total - tailLines; i < total; i++ {
		if !strings.Contains(got, lines[i]) {
			t.Errorf("expected tail line %d (%q) to be present", i, lines[i])
		}
	}

	// Middle lines should be absent
	for i := headLines; i < total-tailLines; i++ {
		if strings.Contains(got, lines[i]) {
			t.Errorf("expected middle line %d (%q) to be absent", i, lines[i])
		}
	}
}

func TestTruncateExecOutput_EmptyString(t *testing.T) {
	got := TruncateExecOutput("")
	if got != "" {
